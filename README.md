...
### v2.0.0
*   **コードリファクタリング**: 設定値を`CONFIG`オブジェクトに集約し、調整を容易にしました。
*   **スクロール復元の最適化**: `setTimeout`から`requestAnimationFrame`に変更し、描画サイクルと同期したスムーズな復元動作を実現。
*   **JSDocコメント追加**: 各関数にドキュメントコメントを追加し、コードの可読性・保守性を向上。
*   **レガシーコードの削除**: 旧形式（v1）からのデータ移行ロジックを削除し、コードをシンプル化。
